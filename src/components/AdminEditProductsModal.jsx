  ...
  const AdminEditProductsModal = ({ product, onClose, onChange, onSave, onCreate, isEditing }) => {
    ...
    const handleTallaChange = (talla) => {
      ...
      if (tallasActuales.includes(String(talla))) {
        nuevasTallas = tallasActuales.filter((t) => t !== String(talla));
      } else {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center px-4 backdrop-blur-sm bg-black/40" onClick={onClose}>
        <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-md transition-all max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          <h2 className="text-2xl font-bold mb-6 text-center">{isEditing ? "Editar Producto" : "Nuevo Producto"}</h2>
          <div className="space-y-4">
            ...
            <input
              ...
              onChange={e => onChange({ ...product, descripcion: e.target.value })}
              placeholder="Descripción"
              className="w-full border border-gray-300 rounded px-4 py-2"
            />
            <div>
              <label className="block font-semibold mb-2 text-gray-700">Tallas Disponibles</label>
              <div className="flex flex-wrap gap-3">
                {TALLAS_DISPONIBLES.map((talla) => (
                  <label key={talla} className="flex items-center gap-2 text-gray-700 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={getTallasArray(product.tallas).includes(String(talla))}
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            <button
              ...
              className={`px-4 py-2 text-white rounded ${isEditing ? "bg-[#0056A6] hover:bg-[#004c94]" : "bg-blue-600 hover:bg-blue-700"}`}
            >
